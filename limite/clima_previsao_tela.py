...
class ClimaPrevisaoTela(TelaAbstrata):
    ...
    def pega_dados_ver_clima(self):
        ...
        self.__window = sg.Window('Dados para Ver a Previsão', layout, element_justification='c', finalize=True)
        button, values = self.open()
        dados_ver_clima = None
        if button == 'Confirmar':
            dados_ver_clima = {"cpf": values['cpf'], "cidade": values['cidade'].title()}
        else:
            self.close()
        self.close()
    ...
    def mostra_clima(self, dados_clima):
        ...
        self.__window = sg.Window('Dados Climáticos', layout, element_justification='c', finalize=True)
        self.__window.read()
        self.close()
    ...
    def seleciona_cpf(self):
        ...
        self.__window = sg.Window('Apagar Registro de Previsão', layout, element_justification='c', finalize=True)
        button, values = self.open()
        cpf = None
        if button == 'Confirmar':
            cpf = values['cpf']
    ...
    def mostra_log(self, dados_log):
        ...
        self.__window = sg.Window('Histórico de Registro', layout, element_justification='c', finalize=True)
        self.__window.read()
        self.close()
